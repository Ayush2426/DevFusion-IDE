            ...
            <div>
                ...
                <AnimatePresence>
                    {isOpen && (
                        ...
                        >
                            <FileTree items={item.children} onSelectFile={onSelectFile} onContextMenu={onContextMenu} level={level + 1} />
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
    ...
    return (
        <div className="text-sm">
            {Object.values(items).sort((a, b) => a.name.localeCompare(b.name)).map(item => (
                <FileNode key={item.id} item={item} onSelectFile={onSelectFile} onContextMenu={onContextMenu} level={level} />
            ))}
        </div>
    );
};

const FileExplorer = ({ fileTree, onSelectFile, onCreateFile, onCreateFolder, onDeleteItem }) => {
    ...
    const handleContextMenu = (x, y, item) => {
        setContextMenu({
            x, y, item,
            options: [
                { label: 'Delete', icon: <Trash2 size={14} />, action: () => onDeleteItem(item.id), style: 'danger' },
            ],
        });
    };
        ...
        <aside className="w-64 flex-shrink-0 bg-gray-800/50 p-2 flex flex-col">
            <div className="flex justify-between items-center mb-2 p-1">
                ...
                <div className="flex gap-2">
                    <button onClick={onCreateFile} className="text-gray-400 hover:text-white" title="New File"><Plus size={18} /></button>
            ...
            <div className="flex-grow overflow-y-auto">
                {fileTree ? <FileTree items={fileTree} onSelectFile={onSelectFile} onContextMenu={handleContextMenu} /> : <p className="text-gray-500 text-xs p-2">No files yet.</p>}
            </div>
