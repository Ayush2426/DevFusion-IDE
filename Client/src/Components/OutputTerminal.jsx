...
const OutputTerminal = ({ isVisible, onClose, onRunCode, output, isRunning }) => {

    const renderOutput = () => {
        if (isRunning) {
            return <p className="text-yellow-400 flex items-center gap-2"><Loader2 className="animate-spin" /> Running code...</p>
        }
        if (!output) {
            return (
                <>
                    <p className="text-gray-400">$ Welcome to the DevFusion IDE terminal.</p>
                    <p className="text-gray-400">$ Click "Run Code" to execute the active file.</p>
                </>
            )
        }
        return (
            <>
                {output.status && <p className="text-gray-500">Status: {output.status}</p>}
                ...
                {output.compile_output && <pre className="text-yellow-500 whitespace-pre-wrap">Compiler Output: {output.compile_output}</pre>}
                {output.message && <pre className="text-gray-500 whitespace-pre-wrap">{output.message}</pre>}
            </>
        )
    }
            ...
            <div className="h-full flex flex-col">
                ...
                <div className="flex-shrink-0 flex justify-between items-center p-2 bg-gray-800/50 border-b border-white/10">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-sm font-bold">Output / Terminal</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <motion.button
                            onClick={onRunCode}
