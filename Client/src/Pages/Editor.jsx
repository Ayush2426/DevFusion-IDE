...
import { ArrowLeft, BrainCircuit, Bot, Sparkles, Clipboard, Terminal as TerminalIcon, Eye, Loader2 } from 'lucide-react';
...
const Editor = ({ roomId, onExit }) => {
  ...
  useEffect(() => {
    if (!activeFileId && roomData?.fileTree && Object.keys(roomData.fileTree).length > 0) {
      const firstFileId = Object.keys(roomData.fileTree).find(id => roomData.fileTree[id].type === 'file');
  ...
  const handleOpenPreview = () => {
    if (!roomData?.fileTree) { toast.error("No files to preview."); return; }
    let html = '', css = '', js = '';
    ...
    if (!html && !css && !js) { toast.info("No HTML, CSS, or JS files to preview."); return; }
    const previewBlob = new Blob([`<html><head><style>${css}</style></head><body>${html}<script type="module">${js}</script></body></html>`], { type: 'text/html' });
  ...
  const handleRunCode = async () => {
    ...
    if (['html', 'css', 'js'].includes(extension)) { toast.info("Use 'Open Live Preview' for web files."); return; }
    const languageId = getJudge0LanguageId(activeFile.name);
    if (!languageId) { toast.error(`Execution for .${extension} files not supported.`); return; }
  ...
  const handleAiAction = async (action) => {
      ...
      switch(action) {
          ...
          case 'bugs':
              ...
              const codeToAnalyze = selection.empty ? activeFile.content : editorState.sliceDoc(selection.from, selection.to);
    ...
    >
      ...
      <div className="flex-grow flex overflow-hidden relative">
           ...
           <div className="flex-grow space-y-4">
             {isAiLoading ? (
                <button disabled className="w-full flex items-center justify-center gap-3 p-3 bg-gray-600/50 rounded-lg">
                  <Loader2 size={18} className="animate-spin" /><span>Thinking...</span>
                </button>
             ) : (
               <>
       ...
       <footer className="flex-shrink-0 bg-gray-800/50 px-4 py-1.5 flex items-center justify-between text-xs border-t border-white/10 z-20">
         ...
         <div className="flex items-center gap-4">
            <button onClick={handleOpenPreview} className="flex items-center gap-2 hover:text-white transition-colors text-gray-400">
                <Eye size={14} />
